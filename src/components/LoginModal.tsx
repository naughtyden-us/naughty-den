...
import { LoginModalProps, LoginForm, SignupForm } from '@/types';
...
export const LoginModal: React.FC<LoginModalProps> = ({ onClose, onLogin }) => {
  const [activeForm, setActiveForm] = useState<'login-choice' | 'login' | 'signup'>('login-choice');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onLogin(false); // Default to user, not creator
      onClose();
    } catch (error) {
      ...
      <div className="bg-gray-900 rounded-lg p-8 w-full max-w-md mx-4 relative">
        ...
        {activeForm === 'login-choice' && (
          <div className="space-y-4">
            ...
            <button
              onClick={() => { setActiveForm('login'); setFormData(prev => ({ ...prev, isCreator: false })); }}
            ...
            <button
              onClick={() => { setActiveForm('login'); setFormData(prev => ({ ...prev, isCreator: true })); }}
        ...
        {(activeForm === 'login' || activeForm === 'signup') && (
          <>
            <form onSubmit={activeForm === 'login' ? handleLogin : handleSignup} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  {activeForm === 'login' && (
                    <button 
                      type="button"
                      className="absolute top-1/2 right-4 -translate-y-1/2 text-xs text-gray-500 hover:underline"
                    >
                      Forgot
                    </button>
                  )}
                ...
                <div className="flex justify-center space-x-4 text-sm text-white">
                  <label className="flex items-center">
                    <input 
                      ...
                      onChange={() => setFormData(prev => ({ ...prev, isCreator: false }))} 
                      className="mr-2" 
                      disabled={isLoading}
                    />
                    I&apos;m a User
                  ...
                  <label className="flex items-center">
                    <input 
                      ...
                      value="creator" 
                      checked={formData.isCreator} 
                      onChange={() => setFormData(prev => ({ ...prev, isCreator: true }))} 
                      className="mr-2" 
                      disabled={isLoading}
                    />
                    I&apos;m a Creator
              ...
              <button
                type="submit"
                className="w-full py-3 rounded-lg bg-pink-600 hover:bg-pink-700 transition-colors font-bold text-lg disabled:opacity-50 disabled:cursor-not-allowed"
                disabled={isLoading}
              >
                {isLoading ? (
                  <LoadingStates.Button text={activeForm === 'login' ? 'Logging in...' : 'Creating account...'} />
                ) : (
            ...
            <button
              ...
              className="w-full flex items-center justify-center py-3 rounded-lg bg-gray-800 border border-gray-700 hover:bg-gray-700 transition-colors disabled:opacity-50"
              disabled={isLoading}
            >
            ...
            <div className="text-center mt-6 text-sm">
              {activeForm === 'login' ? (
                ...
                <>
                  ...
                  <button 
                    onClick={() => setActiveForm('login-choice')} 
                    className="text-pink-600 font-bold hover:underline"
                  >
                    LOG IN
                  </button>
